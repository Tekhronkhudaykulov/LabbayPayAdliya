...
const PayToCard = () => {
    ...
    console.log(payFormWithoutAmount, 'pay');
     ...
     <>
        ...
        {isPending && (
            ...
            <div className="grid  grid-cols-[65%_1fr] gap-x-[20px]">
                <div className="bg-[#F4F4F4] flex flex-col justify-between rounded-[12px] gap-y-[20px] w-full p-[30px]">
                    ...
                    *Сумма
                    ...
                    <input id="amount"  type="text"  value={formatPrice(form["amount"] || "")}  onFocus={() => setActiveInputKey("amount")}  className="p-3 bg-white h-[75px] rounded-[12px] text-[27px]" />
                    </div>
                    <div className="flex flex-col gap-1">
                        <label htmlFor="card-number" className="text-[30px] mb-[5px] text-gray-700 font-[500]">
                    *Введите номер карты
            ...
            <div className="mt-auto">
                <FooterNav nextTitle="Получить СМС-код" nextClick={() => handleSubmit()} nextDisabled={card_num  !== 16 || card_expire !== 4}/>
            </div>
     </>
    )
}
